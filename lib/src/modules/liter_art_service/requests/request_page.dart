...
class RequestPage extends StatefulWidget {
  const RequestPage({super.key, this.book, required this.isFromApi});
...
class _RequestPageState extends State<RequestPage> {
  ...
  void initState() {
    super.initState();
    if (widget.isFromApi && widget.book != null) {
      ...
      // Define a categoria e idioma com base nos dados da API, se disponíveis
      if (widget.book?.volumeInfo.categories != null) {
        selectedCategory = widget.book!.volumeInfo.categories!.first;
      }
      selectedLanguage = widget.book?.volumeInfo.language ?? 'Outro';
    }
  }
                        ...
                        ElevatedButton(
                          onPressed: () async {
                            if (_formKey.currentState?.validate() ?? false) {
                              if (widget.book?.volumeInfo.imageLinks
                                          .thumbnail ==
                                      null &&
                                  _selectedImage == null) {
  ...
  Future<void> _submitDonation(BuildContext context) async {
    ...
    try {
      await firestoreServiceRequest.createRequest(
        ...
        bookCover: coverImageUrl, // Usa a URL correta da imagem
